...
const HomeScreen=(props) => {
    ...
    const [trendingWordsWithFrequency,setTrendingWordsWithFrequency]=useState([]);
    ...
    const updateTrendingData=async () => {
        try {
            const response=await fetch("http://localhost:8080/searchfrequency/trending");
    ...
    return (
        <Layout style={{minHeight: '100vh',backgroundColor: 'white'}} >
            <Content className="home-screen-div">
                ...
                >
                    <List
                        dataSource={Object.entries(trendingSearchTerms)}
                        renderItem={(item,index) => (
                            <List.Item>
                                <div className='trending-item'>
                                    <img src={trending} />
                                    <span>{item[0].replace(/\b\w/g,(match) => match.toUpperCase())}</span>
                                </div>
                            </List.Item>
                        )}
                    />
                    ...
                    <List
                        ...
                        renderItem={(item,index) => (
                            <List.Item>
                                <div className='trending-item'>
                                    <img src={trending} />
                                    <span>{item[0].replace(/\b\w/g,(match) => match.toUpperCase())} - {item[1]}</span>
                                </div>
                            </List.Item>
                        )}
                    />
